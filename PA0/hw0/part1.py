...
def interlace(evens, odds):
  ...
  evens_rows = np.split(evens, evens.shape[0], axis=0)
  odds_rows = np.split(odds, odds.shape[0], axis=0)

  outimg_rows = []
  for i in xrange(max(len(evens_rows), len(odds_rows))):
    if i >= len(evens_rows):
      outimg_rows += odds_rows[i:]
      break
    if i >= len(odds_rows):
      outimg_rows += evens_rows[i:]
      break
    outimg_rows.append(evens_rows[i])
    outimg_rows.append(odds_rows[i])

  outimg = np.concatenate(outimg_rows, axis=0)
  #---------------------------------------------------------------------------
  return outimg

...
def blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask):
  ...
  for l in xrange(len(gauss_pyr_mask)):
    blended_img = np.ndarray(gauss_pyr_mask[l].shape, dtype=lapl_pyr_white[l].dtype)
    rows, cols = blended_img.shape
    for r in xrange(rows):
      for c in xrange(cols):
        coef = gauss_pyr_mask[l][r, c]
        blended_img[r, c] = coef * lapl_pyr_white[l][r, c] + (1 - coef) * lapl_pyr_black[l][r, c]

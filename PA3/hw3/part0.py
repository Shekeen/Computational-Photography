...
def reduce(image):
  ...
  smoothed_image = scipy.signal.convolve2d(image, generating_kernel(0.4), 'same')

  rows, cols = image.shape
  out = np.zeros(((rows+1)/2, (cols+1)/2), dtype=image.dtype)
  for r in xrange((rows+1)/2):
    for c in xrange((cols+1)/2):
      out[r, c] = smoothed_image[2*r, 2*c]
  # ----------------------------------------------------------------------------
  return out
...
def expand(image):
  ...
  # Insert your code here ------------------------------------------------------
  rows, cols = image.shape
  expanded_image = np.zeros((2*rows, 2*cols), dtype=image.dtype)
  for r in xrange(rows):
    for c in xrange(cols):
      expanded_image[2*r, 2*c] = image[r, c]

  out = scipy.signal.convolve2d(expanded_image, generating_kernel(0.4), 'same') * 4
  # ----------------------------------------------------------------------------
  return out

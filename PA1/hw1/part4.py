...
def transform_xy_theta(dx, dy):
  ...
  theta = np.ndarray(dx.shape, dtype=float, buffer=np.divide(dx, dy))
...
def transform_xy_mag(dx, dy):
  ...
  # Insert your code here -------------------------------------------------------
  mag = np.ndarray(dx.shape, dtype=float, buffer=np.sqrt(np.square(dx.flatten()) + np.square(dy.flatten())))
...
def run_edges(image):
  ...
  grayimage = gaussian_filter(grayimage, 2)

...
def convolve(image, kernel):
  ...
  output = np.ndarray((rows - d + 1, cols - d + 1), dtype=image.dtype)
  radius = (d - 1) / 2

  for row in xrange(radius, rows - radius):
    for col in xrange(radius, cols - radius):
      window = image[row-radius:row+radius+1, col-radius:col+radius+1]
      result = np.sum(window.flatten() * kernel.flatten())
      if result > 255:
        result = 255
      elif result < 0:
        result = 0
      output[row-radius, col-radius] = result
  #---------------------------------------------------------------------------
  return output
